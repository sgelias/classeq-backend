...
def is_fasta(seq):
    ...
    if not seq.name:
        error_info = 'Error, sequence ' + \
            str(seq.no) + ' has no sequence name.'
        print(seq)
        sys.stderr.write(error_info)
        return False
    if -1 != seq.name.find('>'):
        error_info = 'Error, sequence ' + \
            str(seq.no) + ' name has > character.'
        sys.stderr.write(error_info)
        return False
    if 0 == seq.length:
...
def read_fasta_check_dna(f):
    ...
    for e in read_fasta_yield(f):
        ...
        if res:
            ...
            error_info = 'Sorry, sequence ' + \
                str(e.no) + ' has character ' + str(res) + \
...
def is_sequence_list(sequence_list):
    ...
    for e in sequence_list:
        ...
        if res is not True:
            error_info = 'Sorry, sequence ' + \
                str(count) + ' has illegal character ' + str(res) + \
                '.(The character must be A, C, G or T)'
            sys.stderr.write(error_info)
            return False
...
if __name__ == '__main__':
    ...
    phyche_index_dict = convert_phyche_index_to_dict(
